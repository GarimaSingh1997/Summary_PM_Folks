...
def callback_alarm(bot, job):
    ...
    bot.send_message(chat_id=chat id, text='Wait for some min')

def callback_timer(bot, update, job_queue):
    bot.send_message(chat_id=update.message.chat_id,
                      text='Starting!')
    job_queue.run_repeating(callback_alarm, 100, context=update.message.chat_id)
